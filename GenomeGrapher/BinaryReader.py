...
class BinaryReader:
    ...
    def read(self, n_bits=1):
        ...
        for i in range(bytes_required):
            ...
            as_int = int.from_bytes(as_byte, byteorder="big") #cast it to an int
            for i in range(8):
        ...
        for i in range(n_bits):
            final_result += self.current_bits.pop(0) #add the current bit at the 1's place
            final_result << 1 #bit shift by 1 to make room for the next bit
